...
%% So, lets do this
    ...
    %Randomize the data indexes
    [a,~] = size(train);
    aux = randperm(a);
    train = train(aux,:);
    train_target = train_target(aux);
    train_target = repmat(train_target,1,2);
    train_target(:,2) = (train_target(:,2) == 0)';
    ...
    test_target = repmat(test_target,1,2);
    test_target(:,2) = (test_target(:,2) == 0)';
